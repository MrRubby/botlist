...
client.on("messageCreate", async (message) => {
    ...
    if (message.channel.id === botlist$.botadd_channel) {
        ...
        if (botlist$.botadd_limit) {
            ...
            if (botsCount) {
                if (botlist$.botadd_limit <= botsCount.length) {
                    message.delete()
                    setTimeout(async () => {
                        await message.channel.send({ content: `${message.author} __sunucuya bot ekleme limitini doldurmuşsun__.` }).then((msg) => {
                            setTimeout(async () => {
                                await msg.delete()
                            }, 4000);
                        })
        ...
        if (message.content.length < 18) {
            message.delete()
            setTimeout(async () => {
                await message.channel.send({ content: `${message.author} __bu bir bot id'si değil__, lütfen geçerli bir id gir.` }).then((msg) => {
                    setTimeout(async () => {
                        await msg.delete()
                    }, 4000);
                })
        ...
        try {
            ...
            if (userSearch) {
                message.delete()
                setTimeout(async () => {
                    await message.channel.send({ content: `${message.author} __bu bot sunucuda zaten mevcut__, lütfen başka bir id gir.` }).then((msg) => {
                        setTimeout(async () => {
                            await msg.delete()
                        }, 4000);
                    })
            ...
            if (botadd_newinfo) {
                if (botadd_newinfo.botId === message.content) {
                    message.delete()
                    setTimeout(async () => {
                        await message.channel.send({ content: `${message.author} __bu bot ekli veya zaten başvurulmuş__, lütfen başka bir id gir.` }).then((msg) => {
                            setTimeout(async () => {
                                await msg.delete()
                            }, 4000);
                        })
            ...
            message.delete().catch(lourity => { })
            setTimeout(async () => {
                await message.channel.send({ content: `${message.author} __botun onay için iletildi__, en kısa sürede eklenecektir.` }).then((msg) => {
                    setTimeout(async () => {
                        await msg.delete().catch(lourity => { })
                    }, 5000);
                })
            ...
            const row = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        ...
                        .setURL("https://discord.com/api/oauth2/authorize?client_id="+ message.content + "&permissions=0&scope=applications.commands%20bot")
                )
                /*.addComponents(
                    new ButtonBuilder()
                        .setLabel("Full Yetkili")
                        .setEmoji("🛠️")
                        .setStyle(ButtonStyle.Link)
                        .setURL("https://discord.com/api/oauth2/authorize?client_id="+ message.content + "&permissions=0&scope=applications.commands%20bot")
                )*/
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === "deny_form") {
        ...
        const add_bot_id = interaction.message.embeds[0].fields[1].value.replace(/```/g, ''); 
        ...
        if (bot_add) {
            ...
            const bot_deny_update_embed = new EmbedBuilder()
                ...
                .setDescription(`${author_info} adlı kullanıcı'nın ${bot_info} bot başvurusu **${interaction.user.username}** tarafından __${deny_reason}__ sebebiyle **reddedildi**!`)
                .addFields(
                    //{ name: "\u200B", value: `**${interaction.user.username}** tarafından reddedildi`},
                    //{ name: "\u200B", value: `__${deny_reason}__ sebebiyle`,},
                    { name: "\u200B", value: `<t:${Math.floor(Date.now() / 1000)}:R>`}
                )
                .setThumbnail(bot_info.displayAvatarURL())
            ...
            const bot_deny_embed = new EmbedBuilder()
                .setColor("Red")
                .setAuthor({ name: `${author_info.username}`, iconURL: author_info.displayAvatarURL() })
                .setDescription(`${bot_info} bot başvurun <@${interaction.user.id}> tarafından __${deny_reason}__ sebebiyle **reddedildi**!`)
                .addFields(
...
// developer çıkınca bot otomatik atılma sistemi 
